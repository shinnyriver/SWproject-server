...
def upload_photo():
    ...
    # 파일 저장 로직 필요
    photo = Photo(user_id=current_user.id, image_path='path/to/image', description=description, keywords=keywords)
...
def search_photos():
    ...
    results = [{"image_path": photo.image_path, "description": photo.description, "keywords": photo.keywords} for photo in photos]
...
def upload_photo():
    ...
    photo = Photo(user_id=current_user.id, image_path=filepath, description=description, keywords=keywords)
    db.session.add(photo)
    db.session.commit()
    return jsonify({"message": "Photo uploaded successfully"}), 201
